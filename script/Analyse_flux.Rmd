...
seu <- FindNeighbors(seu, dims = 1:7, verbose = F, assay = 'FLUX')  # remplir les dims
seu <- FindClusters(seu , graph.name = 'SCT_snn', verbose = F, resolution = 0.7)  # pas sur 
head(Idents(seu))
...
DimPlot(seu, reduction = "umap", label = TRUE, group.by = "SCT_snn_res.0.7")
...
DimPlot(seu, reduction = "umap", label.box = T, label = T, group.by = "SCT_snn_res.0.7") +
...
cell$flux_cluster <- seu@meta.data$SCT_snn_res.0.7
...
tg_ids <- which(ppp_all > 5e-3) 

sel <- str_replace(rownames(dt_c0)[tg_ids],"M-" ,"M_")
sel

predFlux <- predFlux[,sel]*1000
dim(predFlux)  # 143 M après filtre CV 
colnames(predFlux)
```
...
```{r}
resume <- filter(resume, maxi >= 1)
dim(resume) # 73 modules left
resume$module
sel[which(!(sel %in% resume$module))]
predFlux <- predFlux[,resume$module]
dim(predFlux)
```
...
supermodules <- mouse_module_info[colnames(flux)[-1],7]
supermodules <- str_c(rep("SM", 66), str_trim(supermodules, side = "left"))
for (i in 1:22) {
  print(paste0(paste0("SM", as.character(i)), " ", paste0("SM", as.character(i)) %in% supermodules))
}

mouse_module_info[mouse_module_info[,1][!(mouse_module_info[,1] %in% resume$module)], 7]
...
```{r}
gr <- list()
mod <- c(seq(1, 2, 1), seq(5,18,1), seq(20,22,1))
for (i in mod) {
  data_int <- subset(long, SM == paste0("SM",i))
  gr[[i]] <- ggplot(data_int, aes(x=modules, y=fluxes)) + geom_boxplot() + theme_CA() + ggtitle(paste0("SM",i)) + ylab("Fluxes (*10^3)")
}
...
cell_group <- as.numeric(factor(as.factor(yyy), levels = order))
colSide <- coul[cell_group]
module_group <- as.numeric(factor(as.factor(supermodules), c("SM1", "SM2", "SM5", "SM6", "SM7", "SM8", "SM9", "SM10", "SM11", "SM12", "SM13", "SM14", "SM15", "SM16", "SM17", "SM18", "SM20", "SM21", "SM22")))   #SM a modif en fct des resultats   "SM19",
colMod <- coul11[module_group]
...
## Heatmap
heatmap.2(mat, trace = 'none', breaks = brek, col = hm.colors, RowSideColors=colSide, ColSideColors=colMod, margins=c(0.5,0.5), key = TRUE, symkey = FALSE, density.info = 'none') #, Rowv = F, dendrogram = "column"
legend(0,0.5, legend=order, fill=coul, cex=0.7)
legend(0.37,1, legend=c("SM1", "SM2", "SM5", "SM6", "SM7", "SM8", "SM9", "SM10", "SM11", "SM12", "SM13", "SM14", "SM15", "SM16", "SM17", "SM18", "SM20", "SM21", "SM22"), fill=coul11, cex = 0.6, ncol =7)   # "SM19",
```
